...
void FIG_UpdKey()
{
  uint16_t Keys = KeyStatus & FigKeyMask;
      ...
      do
      {
        ...
        if (mpuIntStatus & MPU_DATA_READY)
        {
          ...
          else
          {
            ...
            SuccessRead++;
            recipNorm = (Roll>0)?Roll:(-1.0*Roll);
            if(_Sel == MPU_FIG1)
            {
              if(recipNorm>(90-RollLim) || recipNorm<(RollLim-90))
              {
                Dot[0] = acc[0]*CalAcc[0] + acc[2]*CalAcc[1];
                Dot[1] = invSqrt(acc[0]*acc[0] + acc[2]*acc[2]) * invSqrt(CalAcc[0]*CalAcc[0] + CalAcc[1]*CalAcc[1]);
                Angle = acos(Dot[0]*Dot[1]) * Ra2De;
                Dprintf("Use Pitch.");
              }
              else
              {
  //              Dot[0] = mag[0]*MagRef[0] + mag[2]*MagRef[1];
  //              Dot[1] = invSqrt(mag[0]*mag[0] + mag[2]*mag[2]) * invSqrt(MagRef[0]*MagRef[0] + MagRef[1]*MagRef[1]);
  //              Angle = acos(Dot[0]*Dot[1]) * Ra2De;
                Angle =  atan2(mag[1],mag[0]);
                if(tempj == 1)
                  Angle -=  atan2(mag2[1],mag2[0]);
                Angle *= Ra2De;
                Dprintf("Use YAW.");
              }
              Angle = 90.0 - Angle;
            ...
            {            
              if(recipNorm>RollLim && recipNorm<(180-RollLim))
              {
  //              Dot[0] = mag[0]*MagRef[0] + mag[2]*MagRef[2];
  //              Dot[1] = invSqrt(mag[0]*mag[0] + mag[2]*mag[2]) * invSqrt(MagRef[0]*MagRef[0] + MagRef[2]*MagRef[2]);
  //              Angle = acos(Dot[0]*Dot[1]) * Ra2De;
                Angle =  atan2(mag[1],mag[0]);
                if(tempj == 1)
                  Angle -=  atan2(mag2[1],mag2[0]);
        ...
        if(tempj != 1)
        {
          AngFil[i][AngInd[i]++] = Angle;
          AngInd[i] %= Filter;
          AngFil[i][Filter] = 0;
          for(k=0;k<Filter;k++)
            AngFil[i][Filter] += AngFil[i][k] * 1.0 / Filter;
          Dprintf("Deg Diff[%d]: %10f", _Sel, Angle);
          
          if(abs((int16_t)(Angle - (SAng[i]&0xff))) > ResendVal )
            NeedReSend++;
          if( ( (SensMask[i]<<SensShift) & mpuTestResult ) == 0)
            break;
          tempj = 1;
        ...
        {
          AngFil2[i][AngInd2[i]++] = Angle;
          AngInd2[i] %= Filter2;
          AngFil2[i][Filter2] = 0;
          for(k=0;k<Filter2;k++)
            AngFil2[i][Filter2] += AngFil2[i][k] * 1.0 / Filter2;
          Dprintf("Deg Diff[%d-2]: %10f", _Sel, Angle);
          
          if(abs((int16_t)(Angle - (SAng[i]>>8))) > ResendVal )
            NeedReSend++;
          tempj = 0;
          break;
        }
