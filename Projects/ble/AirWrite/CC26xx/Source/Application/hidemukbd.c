...
// HID reports.
static void HidEmuKbd_sendReport(uint8_t keycode0, uint8_t keycode1, uint8_t keycode2);
static void HidEmuKbd_sendMouseReport(uint8_t buttons, uint8_t x, uint8_t y, uint8_t wheel);
...
static void HidEmuKbd_processAppMsg(hidEmuKbdEvt_t *pMsg)
{
  ...
  switch (pMsg->hdr.event)
  {
    case HIDEMUKBD_KEY_CHANGE_EVT:
        HidEmuKbd_sendReport(pMsg->hdr.keyboardreport[3], pMsg->hdr.keyboardreport[4], pMsg->hdr.keyboardreport[5]);
        HidEmuKbd_sendReport(KEY_NONE, KEY_NONE, KEY_NONE);
        break;
    case HIDEMUKBD_MOUSE_CHANGE_EVT:
        HidEmuKbd_sendMouseReport(pMsg->hdr.mousereport[0], pMsg->hdr.mousereport[1], pMsg->hdr.mousereport[2], pMsg->hdr.mousereport[3]);
        break;
    case HIDEMUKBD_KEY_MOUSE_CHANGE_EVT:
        HidEmuKbd_sendMouseReport(pMsg->hdr.mousereport[0], pMsg->hdr.mousereport[1], pMsg->hdr.mousereport[2], pMsg->hdr.mousereport[3]);
        HidEmuKbd_sendReport(pMsg->hdr.keyboardreport[3], pMsg->hdr.keyboardreport[4], pMsg->hdr.keyboardreport[5]);
        HidEmuKbd_sendReport(KEY_NONE, KEY_NONE, KEY_NONE);
        break;
...
{
  // Enqueue the event.
  //HidEmuKbd_enqueueMsg(HIDEMUKBD_KEY_CHANGE_EVT, 0, 0);
}
...
/*
  ...
  if (keys & KEY_SELECT)
  {
    ...
    {
      // Key Press.
      HidEmuKbd_sendMouseReport(MOUSE_BUTTON_1, 0x78, 0x78);

      // Key Release.
      // NB: releasing a key press will not propagate a signal to this function,
      // so a "key release" is reported immediately afterwards here.
      HidEmuKbd_sendMouseReport (MOUSE_BUTTON_NONE, 0x00, 0x00);
    }
  }
*/
}
...
static void HidEmuKbd_sendReport(uint8_t keycode0, uint8_t keycode1, uint8_t keycode2)
{
  ...
  buf[1] = 0;         // Reserved
  buf[2] = keycode0;  // Keycode 0
  buf[3] = keycode1;  // Keycode 1
  buf[4] = keycode2;  // Keycode 2
  buf[5] = 0;         // Keycode 3
  buf[6] = 0;         // Keycode 4
  buf[7] = 0;         // Keycode 5
                                  ...
                                  uint8_t oper, uint16_t *pLen, uint8_t *pData)
{
  ...
  else if (oper == HID_DEV_OPER_READ)
  {
    ...
    status = HidKbM_GetParameter(id, type, uuid, &len, pData);
    if (status == SUCCESS)
    {
      *pLen = len;
    }
 ...
 */
uint8_t HidEmuKbd_enqueueMsg(uint8_t event, uint8_t* mousereport, uint8_t* keyboardreport)
{
    uint8_t i;
    hidEmuKbdEvt_t *pMsg;
  ...
  if (pMsg = ICall_malloc(sizeof(hidEmuKbdEvt_t)))
  {
    ...
    for (i = 0; i < 5; i++)
    {
        pMsg->hdr.mousereport[i] = mousereport[i];
    }
    for (i = 0; i < 8; i++)
    {
        pMsg->hdr.keyboardreport[i] = keyboardreport[i];
    }
